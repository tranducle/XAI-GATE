...
def write_dataset_workflow_docs(project_root: Path, summary: Dict[str, Any]) -> None:
    methodology_dir = project_root / "4_Methodology_Design"
    analysis_dir = project_root / "6_Analysis_Results"
    methodology_dir.mkdir(parents=True, exist_ok=True)
    analysis_dir.mkdir(parents=True, exist_ok=True)

    dataset_search = f"""# Dataset Search Results: XAI-Gate Real-Data Anchor

## Requirements Definition

- Data type: tabular network-flow security records with binary labels and attack families.
- Domain: network intrusion detection / cybersecurity.
- Access: open public dataset, script-downloadable without manual credentials.
- Method fit: usable as calibrated score-stream metadata for XAI-SurfaceBench.
- Claim boundary: detector training may support calibration, but detector novelty is not claimed.

## Candidate Evaluation

| Dataset | Fit | Access | Main Risk | Decision |
|---|---|---|---|---|
| UNSW-NB15 | Strong: modern IDS flow records with train/test split and attack categories | Official page plus automatable public mirror | Mirror provenance must be documented | Selected |
| KDDCup99 | Useful legacy sanity anchor | Available through scikit-learn | Too old for top-Q1 reviewer expectations | Keep only as legacy anchor |
| CICIDS2017/CSE-CIC-IDS2018 | Strong but larger and more preprocessing-heavy | Public downloads vary by hosting endpoint | Longer preprocessing/runtime | Future second modern anchor |
| TON_IoT | Relevant IoT/security dataset | Public but heterogeneous sources | Requires broader feature harmonization | Future cross-domain anchor |

## Selected Dataset

- Dataset: UNSW-NB15
- Official source: {summary['official_source']}
- Automatable mirror: {summary['download_mirror']}
- Citation DOI: `{summary['citation_doi']}`
- Train rows used: {summary['train_rows']}
- Test rows used: {summary['score_stream_rows']}
"""
    (methodology_dir / "dataset_search_results.md").write_text(dataset_search, encoding="utf-8")

    selected = f"""# Selected Dataset: UNSW-NB15

UNSW-NB15 is selected as the first modern real-dataset anchor for XAI-SurfaceBench.
It is not used to introduce a new intrusion detector. It is used to produce a
held-out calibrated risk-score stream and label distribution that replace the
simulator's default score-bucket priors during the real-dataset benchmark stage.
...
- Dataset paper DOI: `{summary['citation_doi']}`
- Raw file hashes are stored in `6_Analysis_Results/xai_surfacebench/data/unsw_nb15/unsw_nb15_calibration_summary.json`.

## Preprocessing

- Drop identifier, label, and attack-category columns from model features.
- One-hot encode categorical flow fields.
- Standardize numeric fields.
- Train a balanced linear logistic classifier on the public train split.
- Use held-out test probabilities as calibration scores for XAI-SurfaceBench.

## Boundary

The model is calibration infrastructure only. Detector metrics document score-stream
quality and must not be presented as the paper's contribution.
"""
    (methodology_dir / "selected_datasets.md").write_text(selected, encoding="utf-8")

    bucket_rows = ["| Bucket | Rows | Share | Attack rate | Mean score |", "|---|---:|---:|---:|---:|"]
    for name, stats in summary["bucket_summary"].items():
        bucket_rows.append(
            f"| {name} | {stats['rows']} | {stats['share']:.3f} | {stats['attack_rate']:.3f} | {stats['mean_score']:.3f} |"
        )
    eda = f"""# EDA Preview: UNSW-NB15 Calibration Anchor

## Basic Counts

- Train rows: {summary['train_rows']}
- Test / score-stream rows: {summary['score_stream_rows']}
- Test positive-label rate: {summary['positive_rate']:.3f}
- Mean calibration score: {summary['mean_score']:.3f}

## Score Buckets

{markdown_table(bucket_rows)}

## Top Attack Families

| Attack family | Rows |
|---|---:|
{chr(10).join(f"| {attack} | {count} |" for attack, count in list(summary['attack_family_counts'].items())[:15])}
...
"""
    (analysis_dir / "eda_preview.md").write_text(eda, encoding="utf-8")
...
def main() -> int:
    ...
    config_path = root / "configs" / "unsw_nb15_calibrated_anchor.json"
    write_csv(score_path, rows)
    write_json(summary_path, summary)
    write_config(config_path)
    write_reports(root, summary, score_path, config_path)
    write_dataset_workflow_docs(project_root, summary)
